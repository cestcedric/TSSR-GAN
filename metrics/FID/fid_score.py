...
parser.add_argument('--precompute', action = 'store_true', dest = 'precompute')
parser.set_defaults(precompute = False)
...
def calculate_fid_given_paths(paths, batch_size, device, dims, precompute):
    """Calculates the FID of two paths"""
    if not os.path.exists(paths[0]):
        raise RuntimeError('Invalid path: %s' % paths[0])
    if not os.path.exists(paths[1]) and not precompute:
    ...
    if precompute:
        np.savez(paths[1], mu = m1, sigma = s1)
        return 'saved weights under' + paths[1]
